...
import {
  ...
  type MaterialGeneratorInput,
  type MaterialGeneratorOutput
} from "@/lib/material-generator";

const initialInput: MaterialGeneratorInput = {
  grade: 3,
  subject: "Matematika",
...
export function MaterialGenerator() {
  const [input, setInput] = useState<MaterialGeneratorInput>(initialInput);
  const [result, setResult] = useState<MaterialGeneratorOutput | null>(() =>
    generateMockMaterial(initialInput)
  );

  function update<K extends keyof MaterialGeneratorInput>(
    key: K,
    value: MaterialGeneratorInput[K]
  ) {
    ...
    <div className="grid gap-5 lg:grid-cols-[0.75fr_1.25fr]">
      <form onSubmit={handleSubmit} className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-ink">Buat Materi Digital</h2>
        ...
        <div className="mt-5 grid gap-4">
          ...
          <label className="space-y-2">
            ...
            <select
              ...
              onChange={(event) =>
                update("difficulty", event.target.value as MaterialGeneratorInput["difficulty"])
              }
            >
      ...
      <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="label text-ocean-700">Output Materi</p>
