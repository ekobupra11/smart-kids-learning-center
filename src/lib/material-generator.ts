export type MaterialGeneratorInput = {
  grade: number;
  subject: string;
  chapter: string;
...
export type MaterialGeneratorOutput = {
  summary: string;
  examples: string[];
  exercises: string[];
  homework: string[];
...
export function generateMockMaterial(input: MaterialGeneratorInput): MaterialGeneratorOutput {
  const levelText = {
  ...
  return {
    summary: `Ringkasan materi untuk ${topic}. Materi disusun dengan ${levelText}, cocok untuk sesi bimbel SD dan latihan terarah.`,
    examples: [
